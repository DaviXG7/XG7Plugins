...
public class EntityDataWatcher {
    ...
    public void watch(int index, Object value) {
        ...
        if (Objects.equals(fieldType, "f")) {
            value = Optional.of(
                    XG7Plugins.getMinecraftVersion() >= 17 ?
                            IChatBaseComponent.ChatSerializer.a("{\"text\":\"" + value + "\"}")
                            : NMSUtil.getNMSClass("ChatComponentText").getConstructor(String.class).newInstance(value).getObject()
            );
        }
