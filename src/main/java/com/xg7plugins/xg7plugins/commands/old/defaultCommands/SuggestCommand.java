...
import com.xg7plugins.xg7plugins.commands.old.CommandConfig;
import com.xg7plugins.xg7plugins.commands.old.SubCommand;
...
public class SuggestCommand extends Command {
    ...
    @CommandConfig(enabledIf = "suggest")
    public SuggestCommand() {
        super(
                XG7PluginManager.getPlugin().getName().toLowerCase() + "suggest",
                new ItemStack(Material.ARROW),
                "Sends a plugin suggestion to the creator!",
                XG7PluginManager.getPlugin().getName().toLowerCase() + ".suggest",
                Config.getList("commands",XG7PluginManager.getPlugin().getName().toLowerCase() + "suggest").toArray(new String[0]),
                new SubCommand[0]
        );
    }
    ...
    public void onCommand(org.bukkit.command.Command command, CommandSender sender, String[] args, String label) {
        if (System.currentTimeMillis() < cooldownToSendOtherSuggest) {
            sender.sendMessage("§cYou can only send one suggestion every 15 minutes!");
            return;
        }
        sender.sendMessage("§aYour suggestion will be sent, thank you for your help!");
